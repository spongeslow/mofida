...
export function drawChar(
  ...
  // Mouth expression (speaking flap / frown / surprised "o").
  const speakOpen = (t % 280) < 140;
  for (const [x, y, w, h, c] of getMouth(state, speakOpen)) {
    if (pal[c]) { ctx.fillStyle = pal[c]; ctx.fillRect(ox + x * SC, oy + y * SC, w * SC, h * SC); }
  }
  ...
  if (state === 'startled' || state === 'alert' || state === 'surprised') {
